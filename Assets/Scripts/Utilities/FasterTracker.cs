...
/// </summary>
public class FasterTracker : MonoBehaviour
{
    ...
    public static bool gazeRayValidity;
    ...
    {
        CalibrationCallback callback = new CalibrationCallback(CalibrationReceived);
        IntPtr callbackPtr = Marshal.GetFunctionPointerForDelegate(callback);
        return SRanipal_Eye_API.LaunchEyeCalibration(callbackPtr);
    }
    ...
    {
        return gazeRayValidity;
    }

    public (List<Vector3>, List<float>) GetAcquiredEyeData()
    {
        return (gazePositions, gazeTimes);
    }

    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    public delegate void CalibrationCallback();

    public static void CalibrationReceived()
    {
    }
    ...
    private static void EyeCallback(ref EyeData eye_data)
    {
        ...
        gazeRayValidity = eyeData.verbose_data.combined.eye_data.GetValidity(SingleEyeDataValidity.SINGLE_EYE_DATA_GAZE_DIRECTION_VALIDITY);
        gazePositions.Add(gazeRayValidity ? eyeData.verbose_data.combined.eye_data.gaze_direction_normalized : Vector3.zero);
        rayOrigin = eyeData.verbose_data.combined.eye_data.gaze_origin_mm;
